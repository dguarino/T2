...
from experiments import create_experiments_size_nonoverlapping
...
# Model execution
if True:
    data_store,model = run_workflow('ThalamoCorticalModel', ThalamoCorticalModel, create_experiments_size_nonoverlapping )
    data_store.save()

# or only load pickled data
else:
    setup_logging()
    data_store = PickledDataStore(load=True,parameters=ParameterSet({'root_directory':'ThalamoCorticalModel_data_size_closed_nonoverlapping_____', 'store_stimuli' : False}),replace=True)
