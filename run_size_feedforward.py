...
# or only load pickled data
    ...
    setup_logging()
    # data_store = PickledDataStore(load=True,parameters=ParameterSet({'root_directory':'ThalamoCorticalModel_data_size_feedforward_____', 'store_stimuli' : False}),replace=True)
    data_store = PickledDataStore(load=True,parameters=ParameterSet({'root_directory':'ThalamoCorticalModel_data_size_cx2_feedforward_____', 'store_stimuli' : False}),replace=True)
    ...
    if mpi_comm.rank == MPI_ROOT:
        ...
        # perform_analysis_and_visualization_radius( data_store, atype='size', position=[[.0],[.0],[.0]], radius=[], withPGN=True, withV1=True ):
